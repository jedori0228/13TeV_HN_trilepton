#include "syst_UpDowns_MuMuMu.C"

void make_rootfile_for_limit_MuMuMu(bool newfile=true, bool printsyst=false){
  ...
  for(unsigned int i=0; i<masses.size(); i++){
    ...
    // syst_UpDowns_MuMuMu(int sig_mass, bool printnumber=true, bool forlatex=false, bool inclusive=false, bool makeroot=false)
    NLimit n_limit = syst_UpDowns_MuMuMu(mass, false, false, false, true);
    //NLimit n_limit = syst_UpDowns_MuMuMu(mass, false, false, true, true); //inclusive
  ...
  if(printsyst){
    ...
    for(unsigned int i=0; i<masses.size(); i++){
      cout << "& "<<std::fixed<<std::setprecision(2)<<signal_syst_JER.at(i)<<"\\% ";
    }
    cout << " \\\\" << endl;
    ...
    cout << "~~~Jet Energy Resolution"<<"\t";
    for(unsigned int i=0; i<masses.size(); i++){
      cout << "& "<<std::fixed<<std::setprecision(2)<<prompt_syst_JER.at(i)<<"\\% ";
    }
    cout << " \\\\" << endl;
    ...
    cout << "~~~Jet Energy Resolution"<<"\t";
    for(unsigned int i=0; i<masses.size(); i++){
      cout << "& "<<std::fixed<<std::setprecision(2)<<fake_syst_JER.at(i)<<"\\% ";
    }
    cout << " \\\\" << endl;
