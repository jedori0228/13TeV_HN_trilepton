...
void CutStudy_bjet(){
  ...
  for(unsigned int i=0; i<samples.size(); i++){
    ...
    cout << samples.at(i) << " : (bjet==0) = " << hist->Integral(bin_cut_left, bin_cut_right)/TotalIntegral << endl;
  ...
  for(unsigned int i=0; i<sig_masses.size(); i++){
    ...
    int bin_cut_right = hist->GetXaxis()->FindBin(cut_right);
    double TotalIntegral = hist->Integral(-1,hist->GetXaxis()->GetNbins()+1);
    cout << sig_masses.at(i) << " : (bjet==0) = " << hist->Integral(bin_cut_left, bin_cut_right)/TotalIntegral << endl;
